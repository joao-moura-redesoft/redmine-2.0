...
router.get('/mail/folders', handle(async (req, res) => {
  res.json({ folders: await zimbra.listFolders(req) });
}));

// Lista mensagens de uma pasta (paginado).
router.get('/mail/messages', handle(async (req, res) => {
...
router.get('/mail/search', handle(async (req, res) => {
...
router.post('/mail/send', handle(async (req, res) => {
...
router.get('/mail/calendar', handle(async (req, res) => {
...
router.get('/mail/calendar/_debug', handle(async (req, res) => {
  const { start, end } = req.query;
...
router.get('/mail/messages/:id/attachments/:part', handle(async (req, res) => {
  const session = getSession(req.query.s, 'mail');
  if (!session) return res.status(401).json({ error: 'Sessão de e-mail inválida ou expirada. Reabra a mensagem.' });
  const { data, contentType } = await zimbra.fetchAttachment(session, req.params.id, req.params.part);
