...
router.get(
  ...
  handle(async (req, res) => {
    ...
    const result =
      (fileId &&
        (await tryGet(
          `/index.php/core/preview?fileId=${encodeURIComponent(fileId)}&x=${size}&y=${size}&a=0&forceIcon=0`,
        ))) ||
      (filePath &&
        (await tryGet(
          `/index.php/core/preview.png?file=${encodeURIComponent(filePath)}&x=${size}&y=${size}&a=0`,
        ))) ||
...
<oc:filter-files xmlns:d="DAV:" xmlns:oc="http://owncloud.org/ns" xmlns:nc="http://nextcloud.org/ns">
 ...
 <oc:filter-rules><oc:favorite>1</oc:favorite></oc:filter-rules>
    ...
    try {
      ...
      const doc = xml.parse(data);
      const entries = []
        .concat(doc?.multistatus?.response ?? [])
        .map(entryFromResponse)
        .map((x) => x.entry)
        .filter((e) => e.path);
      res.json(entries);
  ...
  <d:limit><d:nresults>200</d:nresults></d:limit>
    ...
    const run = async (withOrder) => {
      const { data } = await (
        ...
        method: 'SEARCH',
        url: '/remote.php/dav/',
        data: buildBody(withOrder),
        headers: { 'Content-Type': 'application/xml' },
        responseType: 'text',
        transformResponse: (r) => r,
      });
      return []
        .concat(xml.parse(data)?.multistatus?.response ?? [])
        .map(entryFromResponse)
        .map((x) => x.entry)
        .filter((e) => e.path && !e.isDir);
    };
    try {
      let entries;
      try {
        entries = await run(true);
      } catch {
        entries = await run(false);
      } // servidor pode não aceitar orderby
...
router.get(
  ...
  handle(async (req, res) => {
    ...
    try {
      const { data } = await (
        await makeTalk(req)
      ).get(`/ocs/v2.php/apps/files_sharing/api/v1/shares`, { params });
      ...
      for (const s of list) {
        ...
        entries.push({
          ...
          etag: '',
          fileId: s.file_source != null ? String(s.file_source) : undefined,
          hasPreview: false,
          favorite: false,
...
<d:propertyupdate xmlns:d="DAV:" xmlns:oc="http://owncloud.org/ns">
 <d:set><d:prop><oc:favorite>${fav}</oc:favorite></d:prop></d:set>
    ...
    await (
      ...
      method: 'PROPPATCH',
      url: `/remote.php/dav/files/${encodeURIComponent(user)}/${enc}`,
      data: body,
      headers: { 'Content-Type': 'application/xml' },
    });
    res.json({ success: true });
...
router.post(
  ...
  handle(async (req, res) => {
    ...
    await (
      ...
      method: 'MOVE',
      url: `/remote.php/dav/trashbin/${encodeURIComponent(user)}/trash/${encodeURIComponent(id)}`,
      headers: {
        Destination: `${ncUrl}/remote.php/dav/trashbin/${encodeURIComponent(user)}/restore/${encodeURIComponent(id)}`,
      },
    });
    res.json({ success: true });
  }),
...
router.delete(
  ...
  handle(async (req, res) => {
    ...
    await (
      ...
      `/remote.php/dav/trashbin/${encodeURIComponent(user)}/trash/${encodeURIComponent(id)}`,
    );
    res.json({ success: true });
  }),
);
