...
// Limita o proxy de OpenGraph: é um fetch para URL arbitrária fornecida pelo
// usuário, então restringe a taxa para evitar abuso como amplificador de saída.
const ogLimiter = createRateLimiter({
  windowMs: 60 * 1000,
  max: 60,
  message: 'Muitas requisições de preview de link. Aguarde um instante.',
});
...
router.get(
  ...
  handle(async (req, res) => {
    ...
    try {
      ...
      try {
        res.json({ url, title: '', description: '', image: '', siteName: new URL(url).hostname });
      } catch {
...
router.get(
  ...
  handle(async (req, res) => {
    ...
    if (!session)
      return res.status(401).json({ error: 'Sessão inválida ou expirada. Recarregue a página.' });
