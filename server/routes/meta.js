...
const { loadTeamOverrides, loadReferenceTeams, fetchAllMemberships, deriveTeam } = require('../services/teams');
...
router.get('/projects/:id/versions', handle(async (req, res) => {
...
// Trackers
router.get('/trackers', handle(async (req, res) => {
  const { data } = await makeRedmine(req).get('/trackers.json');
  res.json(data);
}));

// Prioridades
router.get('/enumerations/issue_priorities', handle(async (req, res) => {
  const { data } = await makeRedmine(req).get('/enumerations/issue_priorities.json');
  res.json(data);
}));

// Usuário atual
router.get('/users/current', handle(async (req, res) => {
...
router.get('/projects/:id/memberships', handle(async (req, res) => {
  const redmine = makeRedmine(req);
