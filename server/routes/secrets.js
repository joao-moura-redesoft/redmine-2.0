...
router.get('/secrets/status', handle(async (req, res) => {
  const uid = await uidOf(req, res); if (!uid) return;
  const ai = getAi(uid);
  res.json({
    ad: !!getAd(uid),
    ai: {
...
router.put('/secrets/ad', handle(async (req, res) => {
  const uid = await uidOf(req, res); if (!uid) return;
...
router.delete('/secrets/ad', handle(async (req, res) => {
  const uid = await uidOf(req, res); if (!uid) return;
  clearAd(uid);
  res.json({ success: true });
}));
...
router.put('/secrets/ai/:provider', handle(async (req, res) => {
  const uid = await uidOf(req, res); if (!uid) return;
...
router.delete('/secrets/ai/:provider', handle(async (req, res) => {
  const uid = await uidOf(req, res); if (!uid) return;
  const { provider } = req.params;
  if (!AI_PROVIDERS.includes(provider)) return res.status(400).json({ error: 'provider inválido' });
...
router.get('/secrets/totp', handle(async (req, res) => {
  const uid = await uidOf(req, res); if (!uid) return;
  const accounts = getTotp(uid).map(a => {
    let code = '------';
    try { code = generateTOTP(a.secret); } catch { /* semente inválida */ }
    return { id: a.id, name: a.name, code };
  });
  res.json({ accounts, remaining: totpRemaining() });
}));

router.post('/secrets/totp', handle(async (req, res) => {
  const uid = await uidOf(req, res); if (!uid) return;
  ...
  try { generateTOTP(secret); } catch { return res.status(400).json({ error: 'semente inválida' }); }
...
router.delete('/secrets/totp/:id', handle(async (req, res) => {
  const uid = await uidOf(req, res); if (!uid) return;
  setTotp(uid, getTotp(uid).filter(a => a.id !== req.params.id));
  res.json({ success: true });
}));
