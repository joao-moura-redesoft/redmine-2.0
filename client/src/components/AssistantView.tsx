  ...
  if (!hasKey) {
    return (
      <div className="max-w-3xl mx-auto flex flex-col items-center justify-center h-[calc(100vh-200px)] text-center text-slate-400">
        ...
        <p className="text-sm">
          Configure uma chave de IA em{' '}
          <strong className="text-slate-600 dark:text-slate-300">Configurações</strong> para usar o
          assistente.
        </p>
      </div>
    );
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100 dark:border-slate-700">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-slate-400">
            Tarefas, wiki, e-mail e notas · cria nota e lança horas
          </p>
        </div>
        {messages.length > 0 && (
          <button
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
              Pergunte sobre tarefas, wiki, e-mails ou notas.
            </p>
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] ${m.role === 'user' ? '' : 'w-full'}`}>
              ...
              {m.role === 'assistant' && m.trace && m.trace.length > 0 && (
                <div className="flex flex-wrap gap-1 mb-1.5">
                  {m.trace.map((t, ti) => (
                    <span
                      key={ti}
                      className="inline-flex items-center gap-1 text-[10px] text-slate-400 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-full px-2 py-0.5"
                    >
              ...
              <div
                className={`text-sm leading-relaxed rounded-2xl px-3.5 py-2 ${
                  m.role === 'user'
