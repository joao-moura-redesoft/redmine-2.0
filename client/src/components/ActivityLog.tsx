...
export function JournalAttachments({
  ...
  const atts = (attachments ?? []).filter(
    (a) => addedIds.includes(a.id) && !inline.has(a.filename),
  );
...
function labelFor(d: JournalDetail, ctx: LookupCtx): string {
  if (d.property === 'cf') {
    return (
      ctx.issue.custom_fields?.find((c) => String(c.id) === d.name)?.name ?? 'Campo personalizado'
    );
  }
...
function valueFor(d: JournalDetail, raw: string | null, ctx: LookupCtx): string {
  ...
  if (d.property === 'attr') {
    ...
    if (d.name === 'assigned_to_id')
      return ctx.members?.find((m) => m.id === Number(raw))?.name ?? `#${raw}`;
