...
interface AmbiguitiesResult {
  hasIssues: boolean;
  ambiguities: AmbiguityItem[];
}
interface VersionNoteResult {
  notes: string[];
  reasoning: string;
}
...
function triggerAIOnOpen(issueId: number) {
  pendingAIIssueId = issueId;
}
...
function getCachedAmbiguities(issue: {
  id: number;
  attachments?: { id: number }[];
}): AmbiguitiesResult | null {
  try {
...
function setCachedAmbiguities(
  issue: { id: number; attachments?: { id: number }[] },
  data: AmbiguitiesResult,
) {
  try {
    sessionStorage.setItem(AMBIGUITY_CACHE_KEY(issue.id, attachSig(issue)), JSON.stringify(data));
  } catch {
    /* ignore */
  }
}
...
function clearCachedAmbiguities(issue: { id: number; attachments?: { id: number }[] }) {
  try {
    sessionStorage.removeItem(AMBIGUITY_CACHE_KEY(issue.id, attachSig(issue)));
  } catch {
    /* ignore */
  }
}
...
export function IssueAIPanel({ issue, compact = false, onOpen, onInsertNote }: Props) {
  ...
  useEffect(() => {
    ...
    const isClosed = ['fechad', 'cancelad'].some((s) =>
  ...
  const runMode = async (mode: Mode) => {
    ...
    try {
      ...
      if (mode === 'ambiguities') {
        ...
        setResult({
          mode,
          text: a.hasIssues ? `${a.ambiguities.length} pontos ambíguos` : 'Requisitos claros',
          copied: false,
          ambiguities: a,
        });
        return;
    ...
    <div className="space-y-2">
          ...
          >
            ...
            <span>
              <span className="font-semibold">
                {autoAmbiguity.ambiguities.length} ponto
                {autoAmbiguity.ambiguities.length !== 1 ? 's' : ''} ambíguo
                {autoAmbiguity.ambiguities.length !== 1 ? 's' : ''}
              </span>{' '}
              identificado{autoAmbiguity.ambiguities.length !== 1 ? 's' : ''} nos requisitos —
              clique para ver
            </span>
          </button>
          <button
            onClick={() => {
              clearCachedAmbiguities(issue);
              setAutoAmbiguity(null);
              runMode('ambiguities');
            }}
        ...
        {(
          [
            ...
            },
            {
              ...
              label: 'Resumir histórico',
              icon: <FileText size={11} />,
              always: true,
            },
            {
              mode: 'draft' as Mode,
              label: 'Rascunho de nota',
            ...
            {
              mode: 'ambiguities' as Mode,
              label: 'Req. ambíguos',
              icon: <AlertTriangle size={11} />,
              always: true,
            },
            {
              mode: 'versionnote' as Mode,
      ...
      {result && result.mode === 'estimate' && result.complexity ? (
            ...
            <div className="border border-purple-200 dark:border-purple-800 rounded-xl overflow-hidden bg-white dark:bg-slate-900">
              <div className="flex items-center justify-between px-3 py-2 bg-purple-50 dark:bg-purple-900/30 border-b border-purple-100 dark:border-purple-800">
                ...
                <button
                  ...
                  className="p-1 text-slate-300 dark:text-slate-600 hover:text-slate-500 dark:hover:text-slate-400"
                >
                  <X size={13} />
                </button>
              </div>
            ...
            {!result.ambiguities.hasIssues ? (
              ...
              <div className="space-y-3">
                {result.ambiguities.ambiguities.map((a, i) => (
                  <div
                    key={i}
                    className="border border-amber-200 dark:border-amber-800 rounded-lg p-3 space-y-1.5 bg-amber-50/50 dark:bg-amber-900/10"
                  >
                    ...
                    <p className="text-xs text-slate-600 dark:text-slate-300">
                      <span className="font-semibold">Problema:</span> {a.problema}
                    </p>
          ...
          <div className="p-4 space-y-2">
            {result.versionNote.notes.length === 0 ? (
              <p className="text-xs text-slate-400">
                Não foi possível extrair o padrão da descrição.
              </p>
            ) : (
              result.versionNote.notes.map((note, i) => (
                <div key={i} className="group flex items-start gap-2">
                  <code className="flex-1 text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 font-mono text-slate-700 dark:text-slate-300 leading-relaxed">
                    {note}
                  </code>
                  <button
                    onClick={() => {
        ...
        <ResultPanel
          ...
          onInsert={
            (result.mode === 'draft' || result.mode === 'reply') && onInsertNote
              ? () => {
                  onInsertNote(result.text);
                  setResult(null);
                }
              : undefined
          }
