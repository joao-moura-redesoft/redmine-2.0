...
export function JitsiProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { poppedOutRef.current = poppedOut; }, [poppedOut]);
  ...
  useEffect(() => { currentCallRef.current = activeCall || poppedOut; }, [activeCall, poppedOut]);
  ...
  const startCall = useCallback((call: ActiveCall) => {
    ...
    if (popped && popped.room === call.room) {
      try { popped.win?.focus(); } catch { /* ignore */ }
      return;
    }
    setActiveCall(prev => {
  ...
  const closePopout = useCallback(() => {
    setPoppedOut(prev => {
      try { if (prev?.win && !prev.win.closed) prev.win.close(); } catch { /* ignore */ }
      return null;
    });
  }, []);
  ...
  return (
    <JitsiContext.Provider value={{ activeCall, minimized, poppedOut, startCall, endCall, popOut, closePopout, toggleMinimize, setMinimized, recorder }}>
      {children}
      <MeetingSummaryModal 
        isOpen={summaryOpen} 
