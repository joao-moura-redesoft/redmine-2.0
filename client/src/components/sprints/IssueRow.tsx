  ...
  return (
    <button
      ...
      className={`flex-shrink-0 w-4 h-4 rounded border flex items-center justify-center transition-colors ${
        checked ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-300 dark:border-slate-600 hover:border-blue-400'
      }`}
    >
...
export function IssueRow({ issue, onOpen, left, right, closedIds }: {
  issue: Issue;
  ...
  return (
    <div className={`group flex items-center gap-2 px-2.5 py-1.5 rounded-lg border transition-colors ${
      done
      ...
      {done
        ...
        : <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${PRIORITY_DOTS[issue.priority?.name] ?? 'bg-slate-400'}`} title={issue.priority?.name} />}
      <button onClick={() => onOpen?.(issue.id)} className="min-w-0 flex-1 text-left" title={issue.subject}>
        <div className={`text-xs font-medium truncate ${done ? 'text-slate-400 dark:text-slate-500 line-through' : 'text-slate-700 dark:text-slate-200'}`}>
          <span className="text-slate-400 dark:text-slate-500">#{issue.id}</span> {issue.subject}
        </div>
        <div className="text-[10px] text-slate-400 dark:text-slate-500 truncate">{issue.project?.name}</div>
      </button>
      {right}
    </div>
  );
