    ...
    <aside className="w-[340px] flex-shrink-0 border-r border-slate-200 dark:border-slate-700 flex flex-col">
      <div className="p-3 space-y-2 border-b border-slate-100 dark:border-slate-800">
        <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">
          ...
          <span className="text-slate-300 dark:text-slate-600">({backlog.length})</span>
        </div>
        ...
        <div className="flex items-center gap-2 px-3 py-2 bg-blue-50 dark:bg-blue-900/20 border-b border-blue-100 dark:border-blue-900/40">
          ...
          <div className="ml-auto flex items-center gap-1.5">
            ...
            <button
              ...
              className="text-xs px-2 py-1 rounded-md text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700"
            >
              Limpar
            </button>
          </div>
      ...
      <div
        ref={setNodeRef}
        className={`flex-1 overflow-y-auto p-3 space-y-1.5 scrollbar-thin transition-colors ${isOver ? 'bg-blue-50/60 dark:bg-blue-900/10' : ''}`}
      >
        {backlog.length === 0 && (
        ...
        <SortableContext
          items={backlog.map((i) => issueDragId(i.id))}
          ...
          {backlog.map((issue) => (
            <SortableIssueRow
              ...
              right={
                <AddToSprintMenu
                  sprints={sprints}
                  onPick={(sprintId) => onAddSingle(sprintId, issue.id)}
                />
              }
            />
          ))}
