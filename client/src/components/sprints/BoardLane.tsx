...
function SortableSprintCard(props: {
  ...
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({
    id: sprintDragId(props.sprint.id),
  });
    ...
    <section className="mb-4 border-l-2 pl-2 rounded-l" style={{ borderLeftColor: laneBorder }}>
      <header className="flex items-center gap-2 px-1 py-1.5 group">
        ...
        <div className="relative flex-shrink-0">
          <button
            onClick={() => board && setColorOpen((o) => !o)}
            disabled={!board}
            title={board ? 'Cor do projeto' : undefined}
              ...
              <div className="absolute left-0 mt-1 z-20 grid grid-cols-4 gap-1.5 p-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-lg">
                {PALETTE.map((c) => (
                  <button
                    key={c}
                    onClick={() => {
                      updateBoard.mutate({ id: board.id, patch: { color: c } });
                      setColorOpen(false);
                    }}
                    aria-label={`Cor ${c}`}
        ...
        <span className="text-xs text-slate-400 dark:text-slate-500">
          ...
          {agg.total > 0 && ` · ${agg.closed}/${agg.total} tarefas`}
        </span>
        {agg.total > 0 && (
          <div
          ...
          >
            <div
              ...
              style={{ width: `${aggPct}%`, backgroundColor: accent ?? '#3b82f6' }}
            />
          </div>
        )}
        ...
        <div className="ml-auto flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
          <button
            onClick={onNewSprint}
            title="Nova sprint neste projeto"
          ...
          {board && (
            <ConfirmButton
              onConfirm={() => delBoard.mutate(board.id)}
              icon={<Trash2 size={13} />}
      ...
      {!collapsed && (
        <div ref={setLaneRef} className="flex gap-3 overflow-x-auto pb-1 pl-4 scrollbar-thin">
          {sprints.length === 0 ? (
            <button
              onClick={onNewSprint}
              className="text-xs text-slate-400 dark:text-slate-500 italic px-3 py-6 rounded-xl border border-dashed border-slate-200 dark:border-slate-700 hover:border-blue-300 hover:text-blue-500 w-full"
            >
              Nenhuma sprint. Clique para criar a primeira.
            </button>
          ) : (
            <SortableContext
              items={sprints.map((s) => sprintDragId(s.id))}
