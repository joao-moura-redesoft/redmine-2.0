...
import { useIssues, useCompletedIssues, useToReviewIssues, useTimeEntries } from '../hooks/useRedmine';
...
import { CheckCircle2, Check, Clock, AlertTriangle, ListTodo, TrendingUp, Bell, Hourglass, Flag, Archive, Flame, PlayCircle, Sparkles, ArrowRight, X, ClipboardCheck, ClipboardList, Copy, Timer } from 'lucide-react';
...
function StatCard({ icon, label, color, issues, onSelect }: {
  icon: React.ReactNode; label: string; color: string;
  issues: Issue[]; onSelect: (label: string, issues: Issue[]) => void;
}) {
  ...
  return (
    <button
      ...
      className={`bg-white rounded-xl border border-slate-200 p-4 flex items-start gap-3 text-left w-full ${
        clickable ? 'hover:border-blue-300 hover:shadow-sm cursor-pointer transition-all' : 'cursor-default'
      }`}
    >
...
function KpiModal({ label, issues, onClose, onIssueClick }: {
  label: string; issues: Issue[]; onClose: () => void; onIssueClick: (id: number) => void;
}) {
  useEffect(() => {
        ...
        <div className="overflow-y-auto scrollbar-thin p-2">
          {issues.map(issue => {
            ...
            return (
              <button
                key={issue.id}
                onClick={() => { onIssueClick(issue.id); onClose(); }}
              ...
              >
                ...
                <span className="text-sm text-slate-700 group-hover:text-blue-700 truncate flex-1">{issue.subject}</span>
                {issue.assigned_to && (
                  <span className="text-[10px] text-slate-400 flex-shrink-0 max-w-28 truncate">{issue.assigned_to.name}</span>
                )}
...
function StandupModal({ open, completed, onClose, onIssueClick }: {
  open: Issue[]; completed: Issue[]; onClose: () => void; onIssueClick: (id: number) => void;
}) {
  ...
  const done = completed.filter(i => (i.closed_on || i.updated_on || '').split('T')[0] >= yesterday);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto scrollbar-thin p-4 space-y-4">
          {sections.map(s => (
            <div key={s.title}>
              <p className="text-xs font-semibold text-slate-700 mb-1.5">{s.emoji} {s.title} <span className="text-slate-400 font-normal">· {s.list.length}</span></p>
              {s.list.length === 0 ? (
                ...
                <div className="space-y-0.5">
                  {s.list.map(i => (
                    <button
                      key={i.id}
                      onClick={() => { onIssueClick(i.id); onClose(); }}
                      className="w-full text-left flex items-center gap-2 px-2 py-1 rounded hover:bg-blue-50 group"
                    >
                      <span className="text-xs font-medium text-slate-400 flex-shrink-0">#{i.id}</span>
                      <span className="text-xs text-slate-700 group-hover:text-blue-700 truncate">{i.subject}</span>
                    </button>
                  ))}
                </div>
              )}
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-5">
      ...
      {stats.topProjects.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Por projeto</p>
          {stats.topProjects.map(([name, hours]) => (
            <div key={name} className="flex items-center gap-3">
              <span className="text-xs text-slate-600 w-36 truncate flex-shrink-0 text-right">{name}</span>
...
export function Dashboard({ onIssueClick }: Props) {
  ...
  const stats = useMemo(() => {
    ...
    return { openIssues, overdue, reviewToday, reviewOverdue, missing, completedThisWeek, byStatus, byPriority, ageBuckets, emAndamento, stale30 };
  }, [open, completed]);
  ...
  const attention: Issue[] = [
    ...
    ...stats.missing.filter(i => !stats.overdue.includes(i) && !stats.reviewOverdue.includes(i) && !stats.reviewToday.includes(i)),
  ].slice(0, 10);
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            <Archive size={13} />
            {hideArchived ? `Arquivadas ocultas (${archivedCount})` : `Mostrando arquivadas (${archivedCount})`}
          </button>
        </div>
      </div>
      ...
      {newToday.length > 0 ? (
        <div className="rounded-xl border border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 p-4">
          <div className="flex items-center gap-2 mb-2.5">
            ...
            <span className="text-xs text-blue-500">— tarefas novas ou recém-atribuídas a você</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
            {newToday.slice(0, 6).map(issue => (
              ...
              >
                ...
                <span className="text-xs font-medium text-slate-400 flex-shrink-0">#{issue.id}</span>
                <span className="text-xs text-slate-700 group-hover:text-blue-700 truncate flex-1">{issue.subject}</span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-3">
        <StatCard icon={<ListTodo size={18} className="text-blue-600" />} color="bg-blue-50"
          label="Tarefas abertas" issues={stats.openIssues} onSelect={showKpi} />
        <StatCard icon={<PlayCircle size={18} className="text-cyan-600" />} color="bg-cyan-50"
          label="Em andamento agora" issues={stats.emAndamento} onSelect={showKpi} />
        <StatCard icon={<ClipboardCheck size={18} className="text-violet-600" />} color="bg-violet-50"
          label="Para eu revisar" issues={toReview ?? []} onSelect={showKpi} />
        <StatCard icon={<CheckCircle2 size={18} className="text-green-600" />} color="bg-green-50"
          label="Concluídas na semana" issues={stats.completedThisWeek} onSelect={showKpi} />
        <StatCard icon={<Flame size={18} className="text-rose-600" />} color="bg-rose-50"
          label="Paradas +30 dias" issues={stats.stale30} onSelect={showKpi} />
        <StatCard icon={<AlertTriangle size={18} className="text-red-600" />} color="bg-red-50"
          label="Prazo vencido" issues={stats.overdue} onSelect={showKpi} />
        <StatCard icon={<Bell size={18} className="text-orange-600" />} color="bg-orange-50"
          label="Revisão atrasada" issues={stats.reviewOverdue} onSelect={showKpi} />
        <StatCard icon={<Clock size={18} className="text-amber-600" />} color="bg-amber-50"
          label="Campos faltando" issues={stats.missing} onSelect={showKpi} />
      </div>
          ...
          {sortedStatus.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {sortedStatus.map(([name, count]) => (
                <div key={name} className="flex items-center gap-3">
                  <span className="text-xs text-slate-600 w-40 truncate text-right flex-shrink-0">{name}</span>
          ...
          {sortedPriority.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {sortedPriority.map(([name, count]) => (
                <div key={name} className="flex items-center gap-3">
                  <span className="text-xs text-slate-600 w-20 text-right flex-shrink-0">{name}</span>
                  <div className="flex-1 h-5 bg-slate-100 rounded-full overflow-hidden">
                    <div
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          ...
          {stats.openIssues.length === 0 ? (
            ...
            <>
                  ...
                  return (
                    <div key={b.label} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
                      ...
                      <div
                        ...
                        style={{ height: `${(b.count / maxAge) * 100}%`, minHeight: b.count > 0 ? 8 : 0 }}
              ...
              <p className="text-xs text-slate-400 mt-3">Vermelho/laranja = paradas há mais tempo.</p>
            </>
          )}
        </div>
          ...
          {attention.length === 0 ? (
            ...
            <div className="space-y-1">
              {attention.map(issue => {
                ...
                const tag = rev === 'overdue' ? 'revisão atrasada'
                  : rev === 'today' ? 'revisar hoje'
                  : issue.due_date && issue.due_date < new Date().toISOString().split('T')[0] ? 'prazo vencido'
                  : miss.length ? `${miss.length} campo(s) faltando`
                  : '';
                return (
                  <button
                    key={issue.id}
                  ...
                  >
                    <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${rev === 'overdue' ? 'bg-orange-500' : rev === 'today' ? 'bg-green-500' : 'bg-red-500'}`} />
