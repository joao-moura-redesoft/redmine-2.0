...
const TEMPLATES: { key: string; label: string; make: () => NotePatch }[] = [
  ...
  { key: 'meeting', label: 'Ata de reunião',  make: () => ({ title: `Ata — ${today()}`, tags: ['reunião'], body: TPL_ATA }) },
  { key: '1on1',    label: '1:1',             make: () => ({ title: `1:1 — ${today()}`, tags: ['1:1'], body: TPL_1ON1 }) },
  { key: 'retro',   label: 'Retrospectiva',   make: () => ({ title: `Retro — ${today()}`, tags: ['retro'], body: TPL_RETRO }) },
];
...
function IssueLinker({ onPick, onClose }: { onPick: (issue: { id: number; subject: string }) => void; onClose: () => void }) {
  ...
  return (
    <div ref={ref} className="absolute z-30 top-full left-0 mt-1 w-72 max-w-[calc(100vw-2rem)] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden">
      ...
      <div className="max-h-56 overflow-y-auto scrollbar-thin">
        ...
        {q.trim().length < 2 && <div className="px-3 py-3 text-xs text-slate-400">Digite ao menos 2 caracteres</div>}
        {results.slice(0, 8).map(issue => (
          <button key={issue.id} onClick={() => onPick(issue)}
            ...
            <span className="font-mono text-[10px] font-bold text-blue-600 dark:text-blue-400 flex-shrink-0">#{issue.id}</span>
            <span className="text-xs text-slate-700 dark:text-slate-200 truncate">{issue.subject}</span>
          </button>
        ))}
      </div>
    </div>
...
function LinkedIssueChip({ id, onOpen, onUnlink }: { id: number; onOpen?: (id: number) => void; onUnlink: () => void }) {
  const { data } = useQuery({
    queryKey: ['note-linked-issue', id],
    queryFn: () => redmineApi.getIssue(id).then(i => ({ id: i.id, subject: i.subject })).catch(() => null),
  ...
  return (
    <span className="inline-flex items-center gap-1.5 pl-2 pr-1 py-1 rounded-lg text-xs bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-300">
      <button onClick={() => onOpen?.(id)} className="inline-flex items-center gap-1 hover:underline" title="Abrir tarefa">
...
function NoteEditor({ note, onIssueClick, projectName, onDuplicate, onAutoDeleteEmpty, allNotes, onSelectNote }: {
  note: Note;
  ...
  const onTitle = (v: string) => {
    setTitle(v); pendingTitle.current = v; setSaving(true);
    if (titleTimer.current) clearTimeout(titleTimer.current);
    titleTimer.current = setTimeout(() => { save({ title: v }); pendingTitle.current = null; }, 500);
  };
  const onBody = (v: string) => {
    setBody(v); pendingBody.current = v; setSaving(true);
    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => { save({ body: v }); pendingBody.current = null; }, 600);
  };
      ...
      <div className="flex items-center justify-end gap-0.5 px-3 pt-2.5 pb-1">
        <span className="mr-auto pl-2 flex items-center gap-1 text-[11px] text-slate-300 dark:text-slate-600">
          {saving
            ...
            : <>Salvo · editado {formatDistanceToNow(new Date(note.updatedAt), { addSuffix: true, locale: ptBR })}</>}
        </span>

        <button onClick={exportMarkdown} title="Exportar como .md"
                className="p-1.5 rounded-md text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
          <Download size={14} />
        </button>
        <button onClick={() => onDuplicate(note)} title="Duplicar nota"
                className="p-1.5 rounded-md text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
          <Copy size={14} />
        </button>
        ...
        <div className="relative">
          <button onClick={() => setShowColors(v => !v)} title="Cor"
                  className="p-1.5 rounded-md text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
            <Palette size={14} />
          </button>
          {showColors && (
            <div className="absolute z-30 top-full right-0 mt-1 flex items-center gap-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl p-2">
              {COLORS.map(c => (
                <button key={c.key} title={c.key}
                        onClick={() => { patch({ color: note.color === c.key ? null : c.key }); setShowColors(false); }}
                        className={`w-4 h-4 rounded-full ${c.dot} transition-transform hover:scale-125 ${
                          note.color === c.key ? 'ring-2 ring-offset-1 ring-slate-400 dark:ring-offset-slate-800' : ''
                        }`} />
              ))}
            </div>
          )}
        ...
        <button onClick={() => patch({ pinned: !note.pinned })}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-thin">
        <div className="max-w-2xl mx-auto px-8 pb-16 pt-2">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 mb-5 text-slate-400">
            {note.linkedIssueId ? (
              <LinkedIssueChip id={note.linkedIssueId} onOpen={onIssueClick}
            ...
            {note.tags.map(t => (
              <span key={t} className="group/tag inline-flex items-center gap-0.5 text-xs text-slate-500 dark:text-slate-400">
                #{t}
                <button onClick={() => removeTag(t)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover/tag:opacity-100 transition-opacity">
                  <X size={10} />
                </button>
              </span>
            ))}
            {showTagInput ? (
              <input autoFocus value={tagInput}
                     ...
                     onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addTag(); } if (e.key === 'Escape') { setShowTagInput(false); setTagInput(''); } }}
          ...
          <RichNoteEditor noteId={note.id} value={note.body} onChange={onBody} onIssueClick={onIssueClick} />
          ...
          {note.linkedIssueId && (
            <div className="mt-8 border-t border-slate-100 dark:border-slate-700 pt-4">
              ...
              <div className="space-y-0.5">
                {checklist.map(it => (
                  <label key={it.id} className="group/cl flex items-center gap-2 text-sm py-0.5">
                    ...
                    <span className={it.done ? 'line-through text-slate-400' : 'text-slate-700 dark:text-slate-200'}>{it.text}</span>
                    <button onClick={() => localChecklists.remove(note.linkedIssueId!, it.id)}
              ...
              <input
                ...
                onKeyDown={e => { if (e.key === 'Enter' && clInput.trim()) { localChecklists.add(note.linkedIssueId!, clInput); setClInput(''); } }}
          ...
          {related.length > 0 && (
            <div className="mt-6 border-t border-slate-100 dark:border-slate-700 pt-4">
              ...
              <div className="space-y-1">
                {related.map(r => (
                  <button key={r.id} onClick={() => onSelectNote(r.id)}
        ...
        <div className="flex items-center justify-end px-4 py-2 border-t border-slate-100 dark:border-slate-700">
          <button onClick={sendAsComment} disabled={sendState === 'sending' || !body.trim()}
                  ...
                  className={`inline-flex items-center gap-1.5 text-xs font-medium rounded-lg px-3 py-1.5 transition-colors disabled:opacity-40 ${
                    sendState === 'sent' ? 'bg-green-50 text-green-600 dark:bg-green-900/20' :
                    sendState === 'error' ? 'bg-red-50 text-red-600 dark:bg-red-900/20' :
...
export function NotesView({ onIssueClick, seed }: {
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      if (newBtnRef.current && !newBtnRef.current.contains(e.target as Node)) setShowTemplates(false);
    };
    document.addEventListener('mousedown', handler);
    return () => document.removeEventListener('mousedown', handler);
  }, [showTemplates]);
  ...
  const noteProjects = useMemo(() => {
    const ids = [...new Set(notes.map(n => n.linkedProjectId).filter((id): id is number => !!id))];
  ...
  const openDailyNote = () => {
    ...
    if (existing) { setSelectedId(existing.id); setSearch(''); }
  ...
  return (
    <div className="flex h-[calc(100vh-170px)] gap-4 max-w-7xl mx-auto">
        ...
        <div className="p-3 border-b border-slate-100 dark:border-slate-700 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <div className="relative flex-shrink-0" ref={newBtnRef}>
              <button onClick={() => setShowTemplates(v => !v)} title="Nova nota"
              ...
              {showTemplates && (
                <div className="absolute z-30 top-full right-0 mt-1 w-44 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden py-1">
                  {TEMPLATES.map(t => (
                    <button key={t.key}
                            onClick={() => { handleNew(t.make()); setShowTemplates(false); }}
            ...
            {allTags.map(t => (
              <button key={t} onClick={() => setActiveTag(activeTag === t ? null : t)}
                      className={`text-[11px] rounded-full px-2 py-0.5 border transition-colors ${
                        activeTag === t ? 'bg-blue-50 border-blue-300 text-blue-700 dark:bg-blue-900/20' : 'border-slate-200 dark:border-slate-700 text-slate-500'
                      }`}>
                #{t}
              </button>
            ))}
            {noteProjects.map(p => (
              <button key={p.id} onClick={() => setActiveProject(activeProject === p.id ? null : p.id)}
                      ...
                      className={`inline-flex items-center gap-1 text-[11px] rounded-full px-2 py-0.5 border transition-colors max-w-[140px] ${
                        activeProject === p.id ? 'bg-indigo-50 border-indigo-300 text-indigo-700 dark:bg-indigo-900/20' : 'border-slate-200 dark:border-slate-700 text-slate-500'
                      }`}>
                <Folder size={10} className="flex-shrink-0" /> <span className="truncate">{p.name}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin">
          ...
          {!isLoading && filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 text-slate-400 px-4 text-center">
              ...
              <p className="text-xs">{notes.length === 0 ? 'Nenhuma nota ainda. Crie a primeira!' : 'Nada encontrado.'}</p>
            </div>
          )}
          {filtered.map(n => (
            <button key={n.id} onClick={() => setSelectedId(n.id)}
                    className={`w-full flex items-stretch gap-0 text-left border-b border-slate-50 dark:border-slate-800 last:border-0 transition-colors group ${
                      selectedId === n.id ? 'bg-blue-50 dark:bg-blue-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-800'
                    }`}>
              ...
              <span className="flex-1 min-w-0 px-3 py-2.5">
                <span className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-semibold text-slate-700 dark:text-slate-200 truncate">{noteTitle(n)}</span>
                </span>
                <span className="block text-[11px] text-slate-400 truncate mt-0.5">
                  {n.body.replace(/[#*_`>-]/g, '').trim().slice(0, 60) || 'Vazia'}
                </span>
                <span className="flex items-center gap-2 mt-1">
                  <span className="text-[10px] text-slate-400">{formatDistanceToNow(new Date(n.updatedAt), { addSuffix: true, locale: ptBR })}</span>
                  {n.linkedIssueId && <span className="text-[10px] font-mono text-blue-500">#{n.linkedIssueId}</span>}
                  {n.tags.slice(0, 2).map(t => <span key={t} className="text-[10px] text-slate-400">#{t}</span>)}
                </span>
              </span>
              <span onClick={e => { e.stopPropagation(); setPendingDelete(n); }}
      ...
      <div className="flex-1 min-w-0 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
        {selected ? (
          <NoteEditor key={selected.id} note={selected} onIssueClick={onIssueClick}
          ...
          <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden"
            ...
            <div className="flex items-start gap-3 p-5">
              ...
              <div className="min-w-0">
                <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-100">Excluir nota</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  Tem certeza que deseja excluir <strong className="text-slate-700 dark:text-slate-200">“{noteTitle(pendingDelete)}”</strong>? Esta ação não pode ser desfeita.
                </p>
              </div>
            </div>
