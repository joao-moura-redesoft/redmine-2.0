...
function PresenceDot({ status, size = 10 }: { status?: UserStatusType; size?: number }) {
  ...
  return (
    <span
      className={`absolute -bottom-0.5 -right-0.5 rounded-full ring-2 ring-white dark:ring-slate-800 ${color}`}
...
function RoomAvatar({
  ...
  if (room.type === 1) {
    return (
      <TalkAvatar actorId={room.name} displayName={room.displayName} size={size} status={status} />
    );
  }
  ...
  return (
    <div
      className="rounded-full overflow-hidden flex-shrink-0 bg-gradient-to-br from-indigo-400 to-indigo-600"
      style={{ width: size, height: size }}
    >
      {src ? (
...
function FullEmojiPicker({
  ...
  const [mart, setMart] = useState<{
    Picker: React.ComponentType<Record<string, unknown>>;
    data: unknown;
  } | null>(null);
  useEffect(() => {
    let active = true;
    Promise.all([import('@emoji-mart/react'), import('@emoji-mart/data')])
      .then(([mod, data]) => {
        if (active) setMart({ Picker: mod.default, data: (data as { default: unknown }).default });
      })
      .catch(() => {});
    return () => {
      active = false;
...
function renderBubbleContent(text: string, isMe: boolean, onIssueClick?: (id: number) => void) {
  ...
  parts.forEach((part, pi) => {
    if (part.type === 'issue') {
      ...
      part.content.split('\n').forEach((line, li, arr) => {
        nodes.push(
          ...renderMarkdown(line, isMe).map((n, ni) => <span key={`${pi}-${li}-${ni}`}>{n}</span>),
        );
...
function CallCard({ room, isMe }: { room: string; isMe: boolean }) {
  ...
  return (
    <div className={`flex items-center gap-2.5 ${isMe ? 'text-white' : 'text-slate-700'}`}>
      ...
      <button
        onClick={() => startCall({ room, title: 'Chamada', kind: 'adhoc' })}
        disabled={here}
...
function messageMentionsMe(msg: TalkMessage, myId: string): boolean {
  return Object.values(msg.messageParameters ?? {}).some(
    (p) => (p.type === 'user' && (p.id === myId || p['mention-id'] === myId)) || p.type === 'call',
  );
}
    ...
    >
      ...
      {!isMe &&
        ...
        (showSender ? (
          <TalkAvatar
            actorId={msg.actorId}
            displayName={msg.actorDisplayName}
            size={24}
            onClick={
              onAvatarClick ? () => onAvatarClick(msg.actorId, msg.actorDisplayName) : undefined
            }
          />
        ) : (
      ...
      <div
        className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[80%] min-w-0 relative`}
      >
        {!isMe && showSender && (
          <button
        ...
        <div id={`talk-msg-${msg.id}`} className="relative rounded-2xl">
          ...
          <div
            onClick={(e) => {
              ...
              if ((e.target as HTMLElement).closest('a,button,input,textarea,audio,video,img'))
                return;
              ...
              <>
                {isImage ? (
                  ...
                  <TalkAudio
                    ...
                    actorId={msg.actorId}
                    isMe={isMe}
                    isVoice={msg.messageType === 'voice-message'}
                  />
                ) : (
                  <button
                    onClick={() =>
                      isPreviewable(file.name ?? '', file.mimetype)
                        ? setFilePreview(true)
                        : downloadTalkFile(file, msg.actorId)
                    }
                    title={
                      isPreviewable(file.name ?? '', file.mimetype)
                        ? 'Visualizar arquivo'
        ...
        />
        <div
          className={`flex items-center gap-1 mt-0.5 px-1 justify-end transition-opacity ${
...
function MessageInput({
  ...
  const sendVoice = async () => {
    ...
    try {
      ...
      const result = await uploadFileToTalk(token, file, '', (pct) => setUploadPct(pct), {
        voiceMessage: true,
      });
  ...
  const ALL_PSEUDO: TalkParticipant = {
    actorId: 'all',
    actorType: 'special',
    displayName: 'Todos',
    participantType: 0,
  };
  const filtered: TalkParticipant[] =
    mentionQuery !== null
      ? [
  ...
  const submit = async () => {
    ...
    if (pendingFile) {
      ...
      setUploadError('');
      try {
        const result = await uploadFileToTalk(token, pendingFile, caption, (pct) =>
          setUploadPct(pct),
        );
        if (result.success) {
...
function ChatWindow({
  ...
  const startVideoCall = useCallback(() => {
    ...
    if (!alreadyHere) {
      send.mutate({
        message: `📞 Iniciei uma chamada de vídeo — entre: ${jitsiRoomUrl(jitsiRoom)}`,
      });
    }
  ...
  const [replyParents, setReplyParents] = useState<
    Record<number, NonNullable<TalkMessage['parent']>>
  >({});
    ...
    async (id: number) => {
      ...
      if (!el) {
        ...
        try {
          ...
          /* sem permissão de contexto — ignora */
        } finally {
          setJumpingTo(null);
        }
  ...
  // Pop-up de perfil de usuário
  const [profileUser, setProfileUser] = useState<{ actorId: string; displayName: string } | null>(
    null,
  );
  ...
  const openDM = useCallback(
    async (userId: string) => {
      try {
        ...
        /* falha silenciosa — usuário pode tentar pela lista */
      }
    },
    [qc, onOpenRoom],
  );
  ...
  const visibleMessages = useMemo(() => {
    ...
    return [...allMessages].reverse().filter((m) => {
  ...
  const handleSend = (text: string, replyId?: number) => {
    if (editTarget) {
      ...
      const parentForTemp = repliedTo
        ...
        : undefined;
      send.mutate(
        { message: text, replyTo: replyId, _parent: parentForTemp, _text: text },
        {
          onSuccess: (data) => {
  ...
  const copyMessage = useCallback((msg: TalkMessage) => {
    navigator.clipboard
      ?.writeText(resolveMessageText(msg))
      .then(() => {
        setCopiedId(msg.id);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-white border-b border-slate-100 flex-shrink-0">
        ...
        {hasNewMsg && (
          <span
            className="w-2 h-2 rounded-full bg-blue-500 animate-pulse flex-shrink-0"
            title="Nova mensagem"
          />
        )}
        {/* Status de conexão */}
            ...
            <>
              ...
              >
                ...
                >
                  ...
                  {muted ? 'Ativar Notificações' : 'Silenciar Sala'}
                </button>
              </div>
            </>
          )}
      ...
      {unreadWhileScrolled > 0 && (
        <div className="relative flex-shrink-0">
          ...
          >
            ↓ {unreadWhileScrolled} nova{unreadWhileScrolled > 1 ? 's' : ''} mensagem
            {unreadWhileScrolled > 1 ? 's' : ''}
          </button>
        </div>
      )}
        ...
        <div ref={contentRef}>
          ...
          {visibleMessages.map((m, idx) => {
            ...
            const grouped =
              !divider &&
              !!prev &&
              prev.actorId === m.actorId &&
              m.timestamp - prev.timestamp < 5 * 60;
...
function GroupInfoPanel({
  ...
  const saveName = async () => {
    const v = name.trim();
    if (!v || v === room.displayName) {
      setEditingName(false);
      return;
    }
    setBusy(true);
    try {
      await renameRoom(room.token, v);
      refresh();
    } finally {
      setBusy(false);
      setEditingName(false);
  ...
  const saveDesc = async () => {
    setBusy(true);
    try {
      await setRoomDescription(room.token, desc.trim());
      refresh();
    } finally {
      setBusy(false);
  ...
  const handleAdd = async (userId: string) => {
    setBusy(true);
    try {
      await addParticipant(room.token, userId);
      refresh();
      setAddQuery('');
    } finally {
      setBusy(false);
  ...
  const handleRemove = async (attendeeId?: number) => {
    ...
    setBusy(true);
    try {
      await removeAttendee(room.token, attendeeId);
      refresh();
    } finally {
      setBusy(false);
    }
  ...
  const handleToggleMod = async (p: TalkParticipant) => {
    ...
    try {
      isMod
        ? await demoteModerator(room.token, p.attendeeId)
        : await promoteModerator(room.token, p.attendeeId);
      refresh();
    } finally {
      setBusy(false);
    }
  ...
  const handleAvatar = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setBusy(true);
    try {
      await uploadRoomAvatar(room.token, file);
      refresh();
    } finally {
      setBusy(false);
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-1.5 justify-end">
                  ...
                  <button
                    onClick={saveDesc}
                    disabled={busy}
                    className="px-2 py-1 text-[11px] bg-blue-600 text-white rounded-lg hover:bg-blue-700"
                  >
                    Salvar
                  </button>
                </div>
          ...
          {confirmLeave ? (
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={handleLeave}
                disabled={busy}
                className="px-2 py-1 text-[11px] bg-red-600 text-white rounded-lg hover:bg-red-700"
              >
                Sair
              </button>
            </div>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin p-3">
          ...
          {tab === 'links' && (
            <>
              {links.length === 0 && (
                <p className="text-xs text-slate-400 text-center py-6">
                  Nenhum link nas mensagens carregadas
                </p>
              )}
                ...
                {links.map((l, i) => (
                  ...
                  >
                    ...
                    <span className="text-[9px] text-slate-400">
                      {l.actor} ·{' '}
                      {format(new Date(l.timestamp * 1000), 'd MMM yyyy', { locale: ptBR })}
                    </span>
                  </a>
                ))}
              </div>
...
function MyStatusMenu({ onClose }: { onClose: () => void }) {
  ...
  const saveMsg = async () => {
    ...
    setBusy(true);
    try {
      await setMyStatusMessage(message.trim());
      refresh();
      onClose();
    } finally {
      setBusy(false);
          ...
          <div className="flex items-center gap-1">
            {myId && (
              ...
              >
                <TalkAvatar
                  actorId={myId}
                  displayName={me?.displayName ?? myId}
                  size={24}
                  status={myStatus}
                />
              </button>
              ...
              >
                <div className="relative">
                  <RoomAvatar
                    room={room}
                    size={36}
                    status={room.type === 1 ? statuses?.get(room.name)?.status : undefined}
                  />
                  {hasAlert && (
...
export function TalkChat({
  ...
  useEffect(() => {
    ...
    if (!room) return;
    setOpenChats((prev) =>
      prev.find((r) => r.token === room.token) ? prev : [...prev, room].slice(-2),
    );
    setMinimized((prev) => {
