...
export function RequiredFieldsModal({ statusName, fields, loading, saving, intro, submitLabel, onCancel, onSubmit }: Props) {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4" onClick={onCancel}>
      ...
      >
          ...
          <div className="flex gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                {intro ?? <>Para mudar para <span className="font-medium">{statusName}</span>, preencha:</>}
              </p>
            </div>
          </div>
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          {!loading && fields.length === 0 && (
            <p className="text-sm text-slate-500">
              Não foi possível identificar os campos automaticamente. Preencha-os no formulário e tente de novo.
            </p>
          )}
        ...
        <div className="flex justify-end gap-2 px-5 py-3 border-t border-slate-100">
          ...
          >
            {saving && <Loader2 size={13} className="animate-spin" />} {submitLabel ?? 'Salvar e mudar status'}
          </button>
        </div>
      </div>
    </div>
...
function FieldInput({ field, value, onChange }: {
  field: EditField; value: string | string[]; onChange: (v: string | string[]) => void;
}) {
  const cls = 'w-full text-sm border border-slate-300 rounded-md px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-300';
  switch (field.type) {
    case 'select':
      return (
    ...
    case 'multiselect': {
      ...
      return (
        <select multiple className={`${cls} min-h-[100px]`} value={arrValue} onChange={e => {
    ...
    case 'textarea':
      return <textarea rows={3} className={`${cls} resize-none`} value={value as string} onChange={e => onChange(e.target.value)} />;
    case 'number':
      return <input type="number" className={cls} value={value as string} onChange={e => onChange(e.target.value)} />;
    default:
