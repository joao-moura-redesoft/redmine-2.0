...
async function test() {
  console.log("\\nTeste apenas long notes (sem number e data)");
  try {
    ...
    console.log("Teste apenas long notes sucesso:", res.status);
  } catch (err) {
    console.log("Teste apenas long notes falhou:", err.response?.status, err.response?.data || err.message);
  }
}
